...
A deterministic (rule-based, no LLM) translator from Splunk SPL to the KQL dialect that
Cribl Search accepts, wrapped in a Capra UI that validates the output against the tenant.

## Layout

- `src/translator/` — standalone module, no React or DOM dependencies.
  - `lexer.ts` splits pipelines and tokenizes (`args` mode for command arguments, `expr` mode for eval).
  - `expr.ts` parses eval expressions into an AST and renders KQL; `FUNCS` maps SPL functions.
  - `search.ts` parses search expressions; `renderScope` emits Cribl's initial stage, `renderWhere` a where predicate.
  - `commands.ts` has one handler per SPL command (`COMMANDS`) and the `GENERATING` set.
  - `index.ts` is `translate(spl, opts)`.
- `src/api.ts` — every Cribl REST call. Search endpoints always use `/m/default_search/...`.
- `src/data/kql-catalog.ts` — snapshot of `GET /m/default_search/search/docs` (operators/functions).
- `scripts/translate.ts` — CLI (`npm run translate -- "<spl>"`, `--json`, `--filters-as-where`).
- `tests/translator.test.ts` — unit tests; `npm test`.

## Verified Cribl Search behaviour (drives translator choices)

- `matches regex` needs a regex literal (`/.../flags`); string patterns containing `/` fail at run
  time with "Invalid regular expression flags" even though the preview endpoint accepts them.
  `extract()`, `extract_all()`, `replace_regex()` take RE2 strings — use `@"..."` verbatim strings.
- `substring(s, start, end)` takes an END index, not a length.
- `timestats` aligns buckets to the search's earliest time; `span=15m@h` aligns to the clock like Splunk.
  `timestats ... by f` already returns one column per series (no pivot needed). Empty buckets are omitted.
- `summarize ... by f` keeps a null group; Splunk drops it. The translator adds `where isnotnull(f)`.
- `extract()` returns `""` on no match; the `rex` handler converts that to null.
- `coalesce()` and `isempty()` treat `""` like null. `strcat()` treats null as `""`.
- Field names that are KQL keywords (`range`, `top`, ...) must be written `["range"]`.
- Array indexing works only on fields (`f[0]`), not on call results; `split(s, d, i)` returns a
  one-element array, so `mvindex(split(...), i)` becomes `tostring(split(s, d, i))`.
- `export to lookup name` must not include `.csv`; `lookup output=` takes one quoted, comma-separated string.
- Macros are invoked as `${name}`; the preview endpoint does not expand them.
- The preview endpoint (`POST /search/preview`) parses and plans but does NOT apply dataset scope,
  filters, `limit` after `summarize` (duplicates rows), window functions (returns nothing) or
  subqueries ("nested- or sub-queries not supported"). Use it as a syntax check only.
- Search jobs: `POST /search/jobs {query, earliest, latest}` → `items[0].id`; poll `GET /search/jobs/:id`
  until `status` is `completed`; results are NDJSON (first line metadata).
...
`src/data/kql-catalog.ts` was generated from the docs bundle: fetch
